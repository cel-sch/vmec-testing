import numpy as np 

imag = 1.0j 

class M_00:
    def __init__(self,eq,ntor):

        #Convention is: 00,01,10,11.
        self.DerBs = [False,False,False,True]




        self.DerPol_DerBs11 = [[0,],[0,],[0,]]

        self.var_D11 = np.zeros(shape=(1,eq.R.shape[0], eq.R.shape[1]), dtype=complex)
        self.var_D11[0] = -1 - eq.shear**2 + 2*eq.r*eq.shear**2 - eq.r**2*eq.shear**2 - 2*eq.alpha*eq.shear*np.sin(eq.r) + 2*eq.alpha*eq.r*eq.shear*np.sin(eq.r) - eq.alpha**2*np.sin(eq.r)**2
        




# Boundary Conditions 
#==============================================================#
class BC_0:
    def __init__(self,grid,eq,ntor):

        self.Axis = np.asarray(['Dirichlet']*grid.Mtot)
        self.Edge = np.asarray(['Dirichlet']*grid.Mtot)


